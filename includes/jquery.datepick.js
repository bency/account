...
$.extend(Datepicker.prototype, {
	...
	defaultRenderer: {
		...
		//month: '<div class="datepick-month">' + '<table><thead>{weekHeader}</thead><tbody>{weeks}</tbody></table></div>',
	...
	_generateMonthSelection: function(inst, year, month, minDate, maxDate, monthHeader) {
		...
		var selector = '<select class="' + this._monthYearClass +
			'" title="' + inst.get('monthStatus') + '" style="width:70px">';
		for (var m = 1; m <= 12; m++) {
		...
		if (yearRange == 'any') {
			selector = '<select class="' + this._monthYearClass + ' ' + this._anyYearClass +
				'" title="' + inst.get('yearStatus') + '" style="width:70px">' +
